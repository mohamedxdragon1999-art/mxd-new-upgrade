// core.js â€” MXD Core: Settings + Engine + Data + PDF + Quality System (Enhanced)
...
window.MXD_PrintQualityAnalyzer = {
  analyzePrintability(grid, settings) {
    ...
    if (E && settings.trimSize) {
      var trim = E.TRIM_SIZES[settings.trimSize];
      if (trim) { pageWidth = Math.round(trim.w * 72); pageHeight = Math.round(trim.h * 72); }
    }
    const analysis = {
      ...
      _source: E ? 'MXD_DIMENSION_ENGINE' : 'builtin'
    };
  ...
  validateKdpCompliance(settings) {
    var E = window.MXD_DIMENSION_ENGINE;
    if (E) {
      var result = E.validateKDP({
        trimSize: settings.trimSize || 'letter',
        pageCount: settings.pageCount || 150,
        bleed: settings.bleed || false,
        bookType: settings.bookType || 'paperback'
      });
      return {
        isCompliant: result.compliant,
        issues: result.checks.filter(function(c){return !c.pass}).map(function(c){return c.name+': '+c.detail}),
        warnings: result.warnings || [],
        trimSizes: Object.keys(E.TRIM_SIZES).filter(function(id){return E.TRIM_SIZES[id].category==='kdp_pb'||E.TRIM_SIZES[id].category==='kdp_hc'}),
        recommendedBleed: 0.125,
        minimumMargins: result.kdp.outsideMarginMin_in,
        gutterTier: result.pageCountTier,
        engineResult: result,
        _source: 'MXD_DIMENSION_ENGINE'
      };
    }
    const compliance = {
      isCompliant: true,
...
(function(){
  ...
  registerTool({ id:'wordsearch', label:'Word Search Generator', icon:'ðŸ”¤', defaultEnabled:true, order:1 });
})();
...
window.WordSearchEngine = {
  ...
  mask(R,C,shape,scale=1.0){
    ...
    if(typeof MXDMaskPipeline!=='undefined'&&MXDMaskPipeline.generate){
      ...
      var templates=typeof SHAPE_TEMPLATES!=='undefined'?SHAPE_TEMPLATES:{};
      var m=MXDMaskPipeline.generate(R,C,shape,scale,svgPaths,templates);
      if(m)return m;
    }
    // Fallback: original math-based mask
    const fallbackMask=Array.from({length:R},()=>Array(C).fill(false));
    ...
    for(let y=0;y<R;y++)for(let x=0;x<C;x++){
      ...
      switch(shape){
        ...
        default:{
          ...
          if(mxdMatch){const catIdx=parseInt(mxdMatch[1],10);const shapeIdx=parseInt(mxdMatch[2],10);if(typeof MXDShapes!=='undefined'&&MXDShapes.getShape){const mxdShape=MXDShapes.getShape(catIdx,shapeIdx);if(mxdShape&&mxdShape.path){const normShape=MXDShapes.normalizeShape?MXDShapes.normalizeShape(mxdShape,C,R):mxdShape;v=MXDShapes.pointInShape?MXDShapes.pointInShape(x,y,normShape):false;break;}}v=true;break;}
          const mxp=/^mxp_(\d+)$/.exec(shape);
          if(mxp){const ik=parseInt(mxp[1],10);if(ik>=1&&ik<=60){v=this.mxpShapeCell(R,C,x,y,ik);break;}}
          const mx=/^shape_mx_(\d+)$/.exec(shape);
          if(mx){const ix=parseInt(mx[1],10);if(ix>=1&&ix<=200){v=this.mxShapeCell(R,C,x,y,ix);break;}}
          v=true;break;
        }
      }
      fallbackMask[y][x]=v;
  ...
  generate(cfg){
    ...
    const mask=this.mask(rows,cols,shape,cfg.shapeScale||1.0);
    ...
    const seed=cfg.seed!==undefined&&cfg.seed!==''&&cfg.seed!==null?this.hashSeed(String(cfg.seed)+'_page_'+(cfg.pageIndex||0)):this.hashSeed(seedInput);
    const rng=this.seeded(seed);
    for(let a=0;a<maxAttempts;a++){
      const grid=this.empty(rows,cols),placements={};
      ...
      if(Object.keys(placements).length===selected.length){
        ...
        return{grid,mask,placements,success:true,wordsPlaced:selected.length};
      }
    }
    const grid=this.empty(rows,cols),placements={};
    for(const w of selected){
    ...
    this.fill(grid,mask,letterCase,rng);
    for(let r=0;r<rows;r++)for(let c=0;c<cols;c++)if(!mask[r][c])grid[r][c]='';
    return{grid,mask,placements,success:Object.keys(placements).length===selected.length,wordsPlaced:Object.keys(placements).length};
  }
};
...
window.TEMPLATES = {
  ...
  notebook_blank:    {n:'ðŸ““ Classic Notebook',      bg:'#fefefe',tc:'#2c2c2c',cc:'#1a1a1a',pb:'#f5f5dc',pc:'#8b4513',ac:'#ddd8c4',ff:'Courier New,monospace',  bw:false,gl:false,gb:false,sh:true},
  notebook_grid:     {n:'ðŸ““ Grid Notebook',          bg:'#ffffff',tc:'#2c2c2c',cc:'#1e40af',pb:'#e3f2fd',pc:'#1565c0',ac:'#bbdefb',ff:'Consolas,monospace',     bw:false,gl:true, gb:true, sh:true},
  notebook_lined:    {n:'ðŸ“ Lined Notebook',        bg:'#fffef8',tc:'#1a1a1a',cc:'#333333',pb:'#fffde7',pc:'#795548',ac:'#d7ccc8',ff:'Times New Roman,serif', bw:false,gl:false,gb:false,sh:true},
  journal_classic:   {n:'ðŸ“” Journal Classic',        bg:'#faf8f5',tc:'#3e2723',cc:'#4e342e',pb:'#efebe9',pc:'#5d4037',ac:'#d7ccc8',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  journal_diary:     {n:'ðŸ““ Personal Diary',          bg:'#fff0f5',tc:'#880e4f',cc:'#ad1457',pb:'#fce4ec',pc:'#c2185b',ac:'#f8bbd0',ff:'Comic Sans MS,sans-serif',bw:false,gl:false,gb:false,sh:true},
  journal_travel:    {n:'âœˆï¸ Travel Journal',         bg:'#e8f5e9',tc:'#1b5e20',cc:'#2e7d32',pb:'#c8e6c9',pc:'#388e3c',ac:'#a5d6a7',ff:'Verdana,sans-serif',    bw:false,gl:false,gb:false,sh:true},
  journal_sketch:    {n:'ðŸŽ¨ Sketch Journal',         bg:'#fff8e1',tc:'#e65100',cc:'#bf360c',pb:'#fff3e0',pc:'#ef6c00',ac:'#ffe0b2',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  planner_weekly:    {n:'ðŸ“… Weekly Planner',          bg:'#ffffff',tc:'#1a237e',cc:'#283593',pb:'#e8eaf6',pc:'#3949ab',ac:'#c5cae9',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  planner_monthly:   {n:'ðŸ“† Monthly Planner',        bg:'#f3e5f5',tc:'#4a148c',cc:'#6a1b9a',pb:'#ede7f6',pc:'#7b1fa2',ac:'#d1c4e9',ff:'Helvetica,sans-serif',  bw:false,gl:false,gb:false,sh:true},
  planner_daily:     {n:'ðŸ“‹ Daily Planner',          bg:'#e3f2fd',tc:'#0d47a1',cc:'#1565c0',pb:'#bbdefb',pc:'#1976d2',ac:'#90caf9',ff:'Segoe UI,sans-serif',    bw:false,gl:false,gb:false,sh:true},
  bullet_dots:       {n:'ðŸ“Œ Bullet Journal Dots',    bg:'#fafafa',tc:'#212121',cc:'#424242',pb:'#f5f5f5',pc:'#616161',ac:'#eeeeee',ff:'Roboto,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  bullet_squares:    {n:'ðŸ“ Bullet Journal Squares', bg:'#ffffff',tc:'#1a1a1a',cc:'#212121',pb:'#f5f5f5',pc:'#424242',ac:'#e0e0e0',ff:'Arial,sans-serif',      bw:true,gl:true,gb:true,sh:false},
  recipe_book:       {n:'ðŸ³ Recipe Book',            bg:'#fff8e1',tc:'#bf360c',cc:'#d84315',pb:'#ffe082',pc:'#ff8f00',ac:'#ffb74d',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  gratitude_journal: {n:'ðŸ™ Gratitude Journal',      bg:'#f3e5f5',tc:'#4a148c',cc:'#6a1b9a',pb:'#ede7f6',pc:'#7b1fa2',ac:'#d1c4e9',ff:'Comic Sans MS,sans-serif',bw:false,gl:false,gb:false,sh:true},
  fitness_log:       {n:'ðŸ’ª Fitness Log',            bg:'#e8f5e9',tc:'#1b5e20',cc:'#2e7d32',pb:'#c8e6c9',pc:'#388e3c',ac:'#a5d6a7',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  budget_tracker:    {n:'ðŸ’° Budget Tracker',         bg:'#fce4ec',tc:'#880e4f',cc:'#ad1457',pb:'#f8bbd0',pc:'#c2185b',ac:'#f48fb1',ff:'Courier New,monospace',  bw:false,gl:false,gb:false,sh:true},
  study_notes:       {n:'ðŸ“š Study Notes',           bg:'#e3f2fd',tc:'#0d47a1',cc:'#1565c0',pb:'#bbdefb',pc:'#1976d2',ac:'#90caf9',ff:'Times New Roman,serif', bw:false,gl:false,gb:false,sh:true},
  science_lab:       {n:'ðŸ”¬ Science Lab',            bg:'#e0f7fa',tc:'#006064',cc:'#00838f',pb:'#b2ebf2',pc:'#0097a7',ac:'#80deea',ff:'Consolas,monospace',    bw:false,gl:false,gb:false,sh:true},
  math_workbook:     {n:'ðŸ“ Math Workbook',          bg:'#ffffff',tc:'#1a1a1a',cc:'#212121',pb:'#f5f5f5',pc:'#424242',ac:'#e0e0e0',ff:'Arial,sans-serif',      bw:true,gl:true,gb:true,sh:false},
  handwriting_practice: {n:'âœï¸ Handwriting Practice', bg:'#fffef8',tc:'#2c2c2c',cc:'#1a1a1a',pb:'#fffde7',pc:'#795548',ac:'#d7ccc8',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  bible_study:       {n:'ðŸ“– Bible Study',           bg:'#f5f5dc',tc:'#3e2723',cc:'#4e342e',pb:'#efebe9',pc:'#5d4037',ac:'#d7ccc8',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  doodle_book:       {n:'ðŸŽ¨ Doodle Book',           bg:'#fff8e1',tc:'#e65100',cc:'#bf360c',pb:'#fff3e0',pc:'#ef6c00',ac:'#ffe0b2',ff:'Comic Sans MS,sans-serif',bw:false,gl:false,gb:false,sh:true},
  maze_adventure:    {n:'ðŸŒŸ Maze Adventure',        bg:'#e8eaf6',tc:'#283593',cc:'#3949ab',pb:'#c5cae9',pc:'#5c6bc0',ac:'#9fa8da',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  coloring_page:     {n:'ðŸ–ï¸ Coloring Page',         bg:'#ffffff',tc:'#212121',cc:'#424242',pb:'#f5f5f5',pc:'#616161',ac:'#eeeeee',ff:'Arial,sans-serif',     bw:true,gl:false,gb:false,sh:false},
  scrapbook:         {n:'ðŸ“· Scrapbook',            bg:'#fffde7',tc:'#e65100',cc:'#bf360c',pb:'#ffe082',pc:'#ff8f00',ac:'#ffb74d',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  memory_book:       {n:'ðŸ’ Memory Book',           bg:'#fce4ec',tc:'#880e4f',cc:'#ad1457',pb:'#f8bbd0',pc:'#c2185b',ac:'#f48fb1',ff:'Comic Sans MS,sans-serif',bw:false,gl:false,gb:false,sh:true},
  habit_tracker:     {n:'âœ… Habit Tracker',          bg:'#e8f5e9',tc:'#1b5e20',cc:'#2e7d32',pb:'#c8e6c9',pc:'#388e3c',ac:'#a5d6a7',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  goal_tracker:      {n:'ðŸŽ¯ Goal Tracker',          bg:'#e3f2fd',tc:'#0d47a1',cc:'#1565c0',pb:'#bbdefb',pc:'#1976d2',ac:'#90caf9',ff:'Helvetica,sans-serif',  bw:false,gl:false,gb:false,sh:true},
  password_log:      {n:'ðŸ” Password Log',           bg:'#f3e5f5',tc:'#4a148c',cc:'#6a1b9a',pb:'#ede7f6',pc:'#7b1fa2',ac:'#d1c4e9',ff:'Consolas,monospace',    bw:true,gl:false,gb:false,sh:false},
  contact_book:      {n:'ðŸ“‡ Contact Book',          bg:'#fffef8',tc:'#1a1a1a',cc:'#212121',pb:'#f5f5f5',pc:'#424242',ac:'#e0e0e0',ff:'Arial,sans-serif',      bw:false,gl:false,gb:false,sh:true},
  medical_log:       {n:'ðŸ¥ Medical Log',           bg:'#e0f7fa',tc:'#006064',cc:'#00838f',pb:'#b2ebf2',pc:'#0097a7',ac:'#80deea',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  wedding_planner:   {n:'ðŸ’’ Wedding Planner',       bg:'#fff0f5',tc:'#880e4f',cc:'#ad1457',pb:'#fce4ec',pc:'#c2185b',ac:'#f8bbd0',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  pet_record:        {n:'ðŸ¾ Pet Record Book',      bg:'#e8f5e9',tc:'#1b5e20',cc:'#2e7d32',pb:'#c8e6c9',pc:'#388e3c',ac:'#a5d6a7',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  garden_tracker:    {n:'ðŸŒ» Garden Tracker',       bg:'#fff8e1',tc:'#558b2f',cc:'#33691e',pb:'#dcedc8',pc:'#558b2f',ac:'#aed581',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:true},
  workout_diary:     {n:'ðŸ‹ï¸ Workout Diary',        bg:'#263238',tc:'#eceff1',cc:'#cfd8dc',pb:'#37474f',pc:'#546e7a',ac:'#78909c',ff:'Arial,sans-serif',     bw:false,gl:false,gb:false,sh:false},
  reading_log:       {n:'ðŸ“š Reading Log',          bg:'#faf8f5',tc:'#3e2723',cc:'#4e342e',pb:'#efebe9',pc:'#5d4037',ac:'#d7ccc8',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  music_notes:       {n:'ðŸŽµ Music Notes',          bg:'#fff8e1',tc:'#bf360c',cc:'#d84315',pb:'#ffe082',pc:'#ff8f00',ac:'#ffb74d',ff:'Times New Roman,serif', bw:false,gl:false,gb:false,sh:true},
  dream_journal:     {n:'ðŸ’­ Dream Journal',         bg:'#1a1a2e',tc:'#e1bee7',cc:'#ce93d8',pb:'#2d2d44',pc:'#9c27b0',ac:'#7b1fa2',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:false},
  coffee_stain:      {n:'â˜• Coffee Stain',          bg:'#fffef8',tc:'#4e342e',cc:'#6d4c41',pb:'#d7ccc8',pc:'#8d6e63',ac:'#bcaaa4',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  vintage_parchment: {n:'ðŸ“œ Vintage Parchment',    bg:'#f5e6c8',tc:'#4e342e',cc:'#5d4037',pb:'#efebe9',pc:'#8d6e63',ac:'#bcaaa4',ff:'Georgia,serif',         bw:false,gl:false,gb:false,sh:true},
  minimalist_white:  {n:'â¬œ Minimalist White',      bg:'#ffffff',tc:'#000000',cc:'#212121',pb:'#ffffff',pc:'#424242',ac:'#e0e0e0',ff:'Helvetica,sans-serif',  bw:false,gl:false,gb:false,sh:true},
  minimalist_black:  {n:'â¬› Minimalist Black',      bg:'#121212',tc:'#ffffff',cc:'#e0e0e0',pb:'#1e1e1e',pc:'#9e9e9e',ac:'#424242',ff:'Helvetica,sans-serif',  bw:false,gl:false,gb:false,sh:true},
  gradient_rose:     {n:'ðŸŒ¹ Gradient Rose',        bg:'#1a1a1a',tc:'#f8bbd0',cc:'#f48fb1',pb:'rgba(244,143,177,.1)',pc:'#ec407a',ac:'rgba(240,98,146,.2)',ff:'Arial,sans-serif',   bw:false,gl:false,gb:false,sh:false},
  gradient_ocean:    {n:'ðŸŒŠ Gradient Ocean',       bg:'#0a1628',tc:'#80deea',cc:'#4dd0e1',pb:'rgba(77,208,225,.1)',pc:'#00bcd4',ac:'rgba(0,188,212,.2)',ff:'Arial,sans-serif',   bw:false,gl:false,gb:false,sh:false},
  gradient_forest:   {n:'ðŸŒ² Gradient Forest',      bg:'#0d1f0d',tc:'#a5d6a7',cc:'#81c784',pb:'rgba(129,199,132,.1)',pc:'#4caf50',ac:'rgba(76,175,80,.2)',ff:'Arial,sans-serif',   bw:false,gl:false,gb:false,sh:false},
};

window.SHAPES = [
  {id:'square',ico:'â– ',n:'Square',g:'Basic'},{id:'rectangle',ico:'â–­',n:'Rectangle',g:'Basic'},
  {id:'cat',ico:'ðŸ±',n:'Cat',g:'Animals',organic:true},{id:'dog',ico:'ðŸ¶',n:'Dog',g:'Animals',organic:true},{id:'bird',ico:'ðŸ¦',n:'Bird',g:'Animals',organic:true},{id:'fish',ico:'ðŸŸ',n:'Fish',g:'Animals',organic:true},{id:'bear',ico:'ðŸ»',n:'Bear',g:'Animals',organic:true},{id:'elephant',ico:'ðŸ˜',n:'Elephant',g:'Animals',organic:true},{id:'lion',ico:'ðŸ¦',n:'Lion',g:'Animals',organic:true},{id:'whale',ico:'ðŸ‹',n:'Whale',g:'Animals',organic:true},{id:'shark',ico:'ðŸ¦ˆ',n:'Shark',g:'Animals',organic:true},{id:'dolphin',ico:'ðŸ¬',n:'Dolphin',g:'Animals',organic:true},{id:'snake',ico:'ðŸ',n:'Snake',g:'Animals',organic:true},{id:'butterfly',ico:'ðŸ¦‹',n:'Butterfly',g:'Animals',organic:true},{id:'bee',ico:'ðŸ',n:'Bee',g:'Animals',organic:true},{id:'owl',ico:'ðŸ¦‰',n:'Owl',g:'Animals',organic:true},{id:'penguin',ico:'ðŸ§',n:'Penguin',g:'Animals'},{id:'turtle',ico:'ðŸ¢',n:'Turtle',g:'Animals',organic:true},{id:'rabbit',ico:'ðŸ°',n:'Rabbit',g:'Animals',organic:true},{id:'fox',ico:'ðŸ¦Š',n:'Fox',g:'Animals',organic:true},{id:'wolf',ico:'ðŸº',n:'Wolf',g:'Animals',organic:true},{id:'zebra',ico:'ðŸ¦“',n:'Zebra',g:'Animals',organic:true},{id:'giraffe',ico:'ðŸ¦’',n:'Giraffe',g:'Animals',organic:true},{id:'panda',ico:'ðŸ¼',n:'Panda',g:'Animals',organic:true},{id:'koala',ico:'ðŸ¨',n:'Koala',g:'Animals',organic:true},{id:'deer',ico:'ðŸ¦Œ',n:'Deer',g:'Animals',organic:true},{id:'frog',ico:'ðŸ¸',n:'Frog',g:'Animals',organic:true},{id:'octopus',ico:'ðŸ™',n:'Octopus',g:'Animals',organic:true},{id:'ladybug',ico:'ðŸž',n:'Ladybug',g:'Animals',organic:true},
  {id:'tree',ico:'ðŸŒ²',n:'Tree',g:'Nature',organic:true},{id:'mountain',ico:'â›°',n:'Mountain',g:'Nature',organic:true},{id:'sun',ico:'â˜€ï¸',n:'Sun',g:'Nature',organic:true},{id:'leaf',ico:'ðŸƒ',n:'Leaf',g:'Nature',organic:true},{id:'flower',ico:'ðŸŒ¸',n:'Flower',g:'Nature',organic:true},{id:'cactus',ico:'ðŸŒµ',n:'Cactus',g:'Nature',organic:true},{id:'mushroom',ico:'ðŸ„',n:'Mushroom',g:'Nature',organic:true},{id:'volcano',ico:'ðŸŒ‹',n:'Volcano',g:'Nature',organic:true},{id:'island',ico:'ðŸ',n:'Island',g:'Nature',organic:true},{id:'cloud',ico:'â˜',n:'Cloud',g:'Nature',organic:true},{id:'moon',ico:'â˜½',n:'Moon',g:'Nature',organic:true},{id:'star',ico:'â­',n:'Star',g:'Nature',organic:true},{id:'snowflake',ico:'â„',n:'Snowflake',g:'Nature',organic:true},{id:'rainbow',ico:'ðŸŒˆ',n:'Rainbow',g:'Nature',organic:true},{id:'wave',ico:'ðŸŒŠ',n:'Wave',g:'Nature',organic:true},{id:'shell',ico:'ðŸš',n:'Shell',g:'Nature',organic:true},{id:'acorn',ico:'ðŸŒ°',n:'Acorn',g:'Nature',organic:true},{id:'tulip',ico:'ðŸŒ·',n:'Tulip',g:'Nature',organic:true},{id:'rose',ico:'ðŸŒ¹',n:'Rose',g:'Nature',organic:true},{id:'palm',ico:'ðŸŒ´',n:'Palm',g:'Nature',organic:true}
];

window.SHAPE_GRP_ORDER = ['Basic','Animals','Nature'];
...
window.PRESETS = {
  ...
  Drinks: ['WATER','JUICE','MILK','COFFEE','TEA','SODA','LEMONADE','ICEDTEA','SMOOTHIE','SHAKE','MILKSHAKE','FRAPPE','LATTÃ‰','CAPPUCCINO','ESPRESSO','AMERICANO','MOCHA','COLD','BREW','HOT','CHOCOLATE','WHIPPED','MOCHA','VANILLA','CARAMEL','HAZELNUT','MOCHA','WHITE','CHOCOLATE','SPICED','APPLE','CRANBERRY','ORANGE','GRAPE','PINEAPPLE','MANGO','PASSION','FRUIT','BLEND','SMOOTHIE','ACAI','BOWL','GREEN','SMOOTHIE','PROTEIN','SMOOTHIE','YOGURT','PARFAIT','GRANOLA','BOWL','AÃ‡AÃ','BOWL'],
  ...
  Shapes: ['CIRCLE','SQUARE','TRIANGLE','RECTANGLE','PENTAGON','HEXAGON','HEPTAGON','OCTAGON','NONAGON','DECAGON','OVAL','DIAMOND','HEART','STAR','CRESCENT','CROSS','ARROW','RING','ARC','SPIRAL','WAVE','ZIG','ZAG','DOT','LINE','CURVE','LOOP','SWIRL','SPIRAL','HELIX','ELLIPSE','PARABOLA','HYPERBOLA','SPIROID','CARDIOID','DELTOID','LIMAÃ‡ON','LEMNISCATE','CISSOID','STROPHOID','TRACTRIX','CATENARY','PARABOLA','HYPERBOLA','ELLIPSE','CIRCLE','OVAL'],
...
window.PdfExport = {
  ...
  drawPage(doc,pz,cfg,idx,isSol){
    ...
    const titlePos=cfg.titlePos||1;const titleAlign=cfg.titleAlign||'center';const titleStyle=cfg.titleStyle||'none';const titleWeight=cfg.titleWeight||800;const titleColorHex=cfg.titleColor||tmpl.tc||'#1a1a2e';const titleColor=bw?[0,0,0]:this.hex2rgb(titleColorHex);const titleBgHex=cfg.titleBg||'';const subtitle=cfg.subtitle||'';const titleFontSize=Math.min(Math.max(cfg.titleFontSize||22,10),36);const titleIndent=cfg.titleIndent||0;const titleGap=cfg.titleGap||0;const titleText=(cfg.title||'Word Search')+(isSol?' â€” Solution Key':'');const alignOption=titleAlign==='left'?'left':titleAlign==='right'?'right':'center';let titleX=alignOption==='center'?W/2:alignOption==='left'?M:W-M;titleX+=titleIndent;const titleY=M+16+titleGap;
    if(titleStyle==='banner'||titleStyle==='boxed'||titleBgHex){const tw=doc.getTextWidth(titleText)+32;const bx=alignOption==='center'?titleX-tw/2:alignOption==='left'?titleX-4:titleX-tw+4;const by=titleY-titleFontSize-4;const bh=titleFontSize+14+(subtitle?titleFontSize*0.55+6:0);if(titleBgHex){const[br,bg,bb]=this.hex2rgb(titleBgHex);doc.setFillColor(br,bg,bb);}else{doc.setFillColor(bw?240:Math.min(255,titleColor[0]+50),bw?240:Math.min(255,titleColor[1]+50),bw?240:Math.min(255,titleColor[2]+50));}if(titleStyle==='boxed'){doc.roundedRect(bx,by,tw,bh,3,3,'F');doc.setDrawColor(...titleColor);doc.setLineWidth(1);doc.roundedRect(bx,by,tw,bh,3,3,'S');}else{doc.rect(0,by,W,bh,'F');}}
    if(titleStyle==='underline'){const tw=doc.getTextWidth(titleText);const ul=alignOption==='center'?titleX-tw/2:alignOption==='left'?titleX:titleX-tw;doc.setDrawColor(...titleColor);doc.setLineWidth(1.5);doc.line(ul,titleY+2,ul+tw,titleY+2);}
    const fontStyle=titleWeight>=700?'bold':'normal';doc.setTextColor(...titleColor);doc.setFont('helvetica',fontStyle);doc.setFontSize(titleFontSize);doc.text(titleText,titleX,titleY,{align:alignOption});
    if(subtitle){const subFS=Math.max(9,Math.round(titleFontSize*0.55));doc.setFont('helvetica','normal');doc.setFontSize(subFS);doc.setTextColor(...titleColor.map(v=>Math.min(255,v+60)));doc.text(subtitle,titleX,titleY+subFS+4,{align:alignOption});}
...
// processor.js â€” MXD Processors: Bulk + Batch + Enterprise + Export (Merged)
...
// system.js â€” MXD System: Offline + Security + Support (Merged v7.0)
...
(function(){
  ...
  console.log(`System modules v${SYSTEM_VERSION} loaded â€” AI Brain + Self-Healing Active`);
})();
...
// quantum-grid-engine.js â€” MXD OMNI-NEXUS v11.0 Core
  ...
  class QuantumGridEngine {
    ...
    async init() {
      ...
      if (this.mode === ENGINE_MODE.JS) {
        console.log(`âš¡ Quantum Engine initialized in JS mode (v${VERSION})`);
      }
    }
    ...
    async initWASM() {
      try {
        ...
        if (typeof WebAssembly !== 'undefined') {
          ...
          this.wasmFunctions = this.createWASMSimulation();
          this.wasmReady = true;
          this.mode = ENGINE_MODE.WASM;
          console.log(`ðŸš€ Quantum Engine initialized in WASM mode (v${VERSION})`);
        }
      } catch (e) {
    ...
    initWebGL() {
      try {
        ...
        if (this.webglContext) {
          ...
          console.log(`ðŸŽ® Quantum Engine initialized in WebGL mode (v${VERSION})`);
        }
      } catch (e) {
    ...
    async generate(params) {
      ...
      try {
        ...
        console.warn('âš¡ GPU/WASM error. Falling back to v8.0 Engine with 0 latency.');
...
// hybrid-grid-engine.js â€” MXD OMNI-NEXUS v11.0 Hybrid Puzzle Engine
...
(function() {
  ...
  class HybridGridEngine {
    ...
    init() {
      console.log(`ðŸ”— Hybrid Grid Engine v${HYBRID_VERSION} â€” Multi-Puzzle Integration`);
    }
    ...
    setPuzzleType(type) {
      if (Object.values(PUZZLE_TYPES).includes(type)) {
        ...
        console.log(`ðŸ”— Puzzle type set to: ${type}`);
      }
    }
